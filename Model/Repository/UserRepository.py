...
#Agregado
    def baja_de_usuario(self, user):
        usuario = False
        lista = []
        with open(self.file_path, "r") as file: #Abre el archivo en modo lectura 
            linea = file.readlines()
        for row in linea:
            user_data = row.strip().split(",")
            if len(user_data) == 6 and user_data[0] == user.username and int(user_data[5]) == 1:
                user_data[5] = "0"  # Marcar el usuario como inactivo
                usuario = True
            lista.append(",".join(user_data) + "\n")
        if usuario:
            with open(self.file_path, "w") as file: #Abre denuevo el archivo y sobreescribe las lineas
